import type { CrawledURL, TableCrawledURL } from '../types/url';
...
export const getCrawledURLs = async (): Promise<TableCrawledURL[]> => {
...
export const bulkCrawl = async (urls: string[]) => {
  const res = await API.post('/crawler/urls/crawl', { urls });
